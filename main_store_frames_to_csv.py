...
# csv
export_csv = True
export_path = get_project_root() + f'outputs/'
# out_timestamp = time.strftime("%Y%m%d_%H%M%S")
# export_path = get_project_root() + f'outputs/{out_timestamp}/'


# video
save_video_out = True
video_out_max_n_frames = 500 # -1: no bound set  # split out video in chunks of n frames
...
out_mgr = VideoSaver(
    OutputSpecsCollection({
        'collage_points': {"size": configs.out_image_size, "name": input_mgr.get_input_id()+"_collage_points"},
    }),
...
while(input_mgr.has_frames()):
    try:
        ...
        if frame_read:
            ...
            if gt_mgr.is_timestamp_valid(input_mgr.get_prev_timestamp()):
                # Compute BEV
                ...
                print("Vehicle pose not available, skipping frame")
        else:
