...
#"""
# disable keras occupy whole memory
...
def create_model_lines():
    ...
    conv5 = Conv2D(126, (3, 3), padding="same", activation="relu")(pool4)
    conv5 = Conv2D(126, (3, 3), padding="same", activation="relu")(conv5)
...
class image_converter:
  ...
  def __init__(self, threshold_param=90, size_param=20):
    ...
    model_lines.load_weights("./features_cnn/cnn1/weight.h5")
  ...
  #
  #   # predict
  #   imgs_t = np.ndarray((1, 256, 512,3), dtype=np.uint8)
  #   img_mask_lines = np.ndarray((1, 256, 512,1), dtype=np.uint8)
  #   imgs_t[0]=img
  ...
  #   size_param_lines =20
  #   threshold_param_lines = 90
  #     #size_param = rospy.get_param("size")
  #     #threshold_param= rospy.get_param("thr")
  #   out = image_converter.cleaner (self,mask_lines, threshold_param_lines,size_param_lines,img)
  #
  # lines_found = img
  ...
  def predict(self, frame):
    ...
    threshold_param_lines = self.threshold_param  # threshold_param_lines = 90
    # size_param = rospy.get_param("size")
    # threshold_param= rospy.get_param("thr")

    out_mask_lines_cleaned = self.cleaner(mask_lines, threshold_param_lines, size_param_lines)#, img)

    # postprocess lines
    out_mask_lines = self.postprocess_lines(out_mask_lines_cleaned)

    out_prediction = self.postprocess_lines(mask_lines)

    return out_mask_lines, out_prediction

  def preprocess_frame(self, frame):
      img = cv2.resize(frame, dsize=(512, 288), interpolation=cv2.INTER_CUBIC)
      img = img[32:, :]
      return img

  def postprocess_lines(self, line_mask):
      line_mask = cv2.copyMakeBorder(line_mask, 32, 0, 0, 0, cv2.BORDER_CONSTANT, None, 0) # add 32px zeros on top
      line_mask = cv2.resize(line_mask, dsize=(self.frame_shape[1], self.frame_shape[0]), interpolation=cv2.INTER_NEAREST) # interpolation=cv2.INTER_CUBIC)
      return line_mask
  ...
  def cleaner (self,img,thr,size=None): #,orig):
    ...
    th2, ret = cv2.threshold(img, thr, 255, cv2.THRESH_BINARY)
